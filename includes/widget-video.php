...
class pulsar_video_widgets extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array('classname' => 'pulsar-videos','description' => esc_attr__('Video Widget - supports Youtube, Vimeo, Dailymotion','medicallinktheme'));
		/* $control_ops = array( 'twitter name' => 'pulsar', 'count' => 3, 'avatar_size' => '32' ); */		
		parent::__construct('pulsar-videos',esc_attr__('[Micro Themes] - Videos','medicallinktheme'),$widget_ops);
...
function form( $instance ) {
		...
		$defaults = array( 
			'title' => esc_attr__('Videos','medicallinktheme'), 
        ...
        <p>
        <label for="<?php echo esc_attr($this->get_field_id( 'type' )); ?>"><?php esc_attr_e('Video Type:', 'medicallinktheme') ?></label>
        <select id="<?php echo esc_attr($this->get_field_id( 'type' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'type' )); ?>" class="widefat">
            <option <?php if ( 'Youtube' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Youtube', 'medicallinktheme') ?></option>
            <option <?php if ( 'Vimeo' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Vimeo', 'medicallinktheme') ?></option>
            <option <?php if ( 'Dailymotion' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Dailymotion', 'medicallinktheme') ?></option>
        </select>
        </p>

		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'id' )); ?>"><?php esc_attr_e('Video ID:', 'medicallinktheme'); ?></label>
