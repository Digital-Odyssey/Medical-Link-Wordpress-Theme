(function($) {
	...
	$('.pm_quick_contact_submit').on('click', function(e) {
		...
		if($('#pm_quick_contact_consent_box').length > 0) {
			reg_consent_box = $('#pm_quick_contact_consent_box').attr('checked') ? 'checked' : 'unchecked';
		}
		  ...
		  if(response) {
			...
			if(response === 'full_name_error') {
				...
				$('#pm_message').addClass('invalid_field');
			  	...
			  	$('#pm_form_response').html(wordpressOptionsObject.consentError);
